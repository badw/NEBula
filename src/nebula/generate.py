...
class InterstitialNEB:
    ...
    def neb_create(
        self,
        init_structure: Optional[Union[Atoms, Structure]],
        final_structure: Optional[Union[Atoms, Structure]],
        nimages: int = 5,
    ) -> list:
        """
        uses ase.mep.NEB to generate interpolated images between an initial and final structure
        Args: 
        init_structure: Optional[Union[Atoms,Structure]] ; ase.atoms.Atoms object or pymatgen.core.Structure object of the initial structure of the pathway
        final_structure: Optional[Union[Atoms,Structure]] ; ase.atoms.Atoms object or pymatgen.core.Structure object of the final structure of the pathway
        nimages: int = 5 ; number of images between init_structure and final_structure
        Returns:
        list of ase.atoms.Atoms images

        """
        if isinstance(init_structure,Structure):
            init_atoms = AseAtomsAdaptor.get_atoms(init_structure)
        else:
            init_atoms = init_structure
    ...
    def gen_structure_from_mapping(
            self,
            ...
            """
            generates an endpoint for a NEB calculation from pymatgen.core.Structure object and given sites 

            Args: 
            bulk_structure: pymtagen.core.Structure ; supercell without a defect 
            site1: pymatgen.core.Site ; site object 
            site2: pymatgen.core.Site ; site object 
            Returns: 
            [Structure[initial point],Structure[final point]] 
            """
    ...
    def get_interstitial_neb_mappings(
        self,
        supercell: Optional[Union[Atoms, Structure]],
        desired_interstitial_specie: str,
        neb_distance_cutoff: int = 3,
        interstitial_gen_kwargs:dict = {'stol':0.5},
    ):
        """
        generate interstitial mappings 

        Args: 
        supercell: Optional[Union[Atoms,Structure]] ; supercell of the host material you want to add a defect to 
        desired_interstitial_specie: str ; i.e. 'Li'
        neb_distance_cutoff: int = 3 
        interstitial_gen_kwargs:dict = {'stol':0.5} ; arguments to pass to doped.DefectsGenerator

        Returns: 
        list 
        """
        # load the supercell 
        struct = copy.deepcopy(supercell)

        warnings.simplefilter('ignore')
        # generate all defects using doped and find interstitials (might be a quicker way to access the interstitials directly)
        ...
        interstitial_keys = [
            x
            for x in list(defect_gen.defect_entries)
            if "{}_i".format(desired_interstitial_specie) in x and "_0" in x
        ]
...
{}
        ...
        for k,v in new_mappings.items():
            i,f = self.gen_structure_from_mapping(supercell,v['sites'][0],v['sites'][1])
